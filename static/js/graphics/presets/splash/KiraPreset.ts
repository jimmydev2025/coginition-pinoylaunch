...
/**
 * Пресет с графикой Киры
 */
export class KiraPreset extends SplashPreset {
    /**
     * Путь до пресета
     * @private
     */
    private static readonly PRESET_PATH = 'kira/';
    ...
    public async preload(): Promise<GLTF> {
        ...
        const paintingPaths = Array(39).fill(0).map((val, index) => `${SplashPreset.TEXTURES_ROOT_PATH + KiraPreset.PRESET_PATH}paintings/${index + 1}`);
        ...
        const robotTextures = await TextureLoader.loadCompressedGroup([
            ...
            'body_emissive',
            'cursor_purple',
        ].map((item) => SplashPreset.TEXTURES_ROOT_PATH + KiraPreset.PRESET_PATH + item));
